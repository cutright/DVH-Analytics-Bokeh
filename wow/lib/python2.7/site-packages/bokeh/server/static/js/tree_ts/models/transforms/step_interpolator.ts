...
export var StepInterpolator = (function(superClass) {
  ...
  StepInterpolator.prototype.compute = function(x) {
    ...
    ind = -1;
    if (this.mode === "after") {
      ind = findLastIndex(this._x_sorted, function(num) {
        return x >= num;
      });
    }
    if (this.mode === "before") {
      ind = findIndex(this._x_sorted, function(num) {
        return x <= num;
      });
    }
    if (this.mode === "center") {
      ...
      mdiff = min(diffs);
      ind = findIndex(diffs, function(num) {
        return mdiff === num;
      });
    }
    if (ind !== -1) {
